#include "MainMenu.h"
#include "Manager.h"

void MainMenu::LoadScene() { // Override for virtual LoadScene
	auto UIManager = UIManager::GetInstance();

	float buttonW = 50.0f + (MeasureText("ALTO", 100) - 200) / 2;

	//UIManager->AddButton({ {(SCREEN_WIDTH - 300) / 2 - 160,  200, 300, 100}, "PLAY", 50, LIGHTPINK, BLACK, []() {
	//	SceneManager::GetInstance()->ChangeScene("Play");
	//} });

	UIManager->AddButton({ {buttonW, 225, 200, 75}, "PLAY", 30, LIGHTPINK, BLACK, []() {
		SceneManager::GetInstance()->ChangeScene("PLAY");
	} });

	UIManager->AddButton({ {buttonW, 350, 200, 75}, "SETTINGS", 30, LIGHTPINK, BLACK, []() {
	SceneManager::GetInstance()->ChangeScene("SETTINGS");
} });

	UIManager->AddButton({ {buttonW, 475, 200, 75}, "QUIT", 30, LIGHTPINK, BLACK, []() {
SceneManager::GetInstance()->ChangeScene("QUIT");
} });

	//UIManager->AddButton({ {(SCREEN_WIDTH - 300) / 2 + 200, 200, 300, 100}, "SETTINGS", 50, LIGHTPINK, BLACK, []() {
	//	SceneManager::GetInstance()->ChangeScene("Settings");
	//} });

	//UIManager->AddButton({ {(SCREEN_WIDTH - 300) / 2, 380, 300, 100}, "QUIT", 50, LIGHTPINK, BLACK, []() {
	//	Manager::GetInstance()->Close();
	//} });

	UIManager->AddText(new Text({ "ALTO", 100, {50, 50}, BLACK }));

	backgroundColor = OFFWHITE;

}

void MainMenu::LoadText() {}